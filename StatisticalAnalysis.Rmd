...
```{r}
# Read in data
dataDF <- read.csv("finalCleanedData.csv", stringsAsFactors=TRUE)
head(dataDF)
...
# It appears that the duplicate-named colleges can be uniquely identified by considering name, state, & city
...
dataDF_14_15$Region <- state.region[match(dataDF_14_15$University.State, state.abb)]
...
dataDF_14_15 %>%
  ggplot( aes(x = Comp.Rate.17_18.BIN, y = Admission.Rate, fill = Comp.Rate.17_18.BIN)) +
  geom_boxplot() +
  ggtitle("Admission Rate per Completion Rate Category")
...
dataDF_14_15_cleanSAT %>%
  ggplot( aes(x = Comp.Rate.17_18.BIN, y = SAT.Average, fill = Comp.Rate.17_18.BIN)) +
  geom_boxplot() +
  ggtitle("Average SAT Score per Completion Rate Category")
...
dataDF_14_15_cleanSize %>%
  ggplot( aes(x = Comp.Rate.17_18.BIN, y = Undergraduate.Enrollment, fill = Comp.Rate.17_18.BIN)) +
  geom_boxplot() +
  ggtitle("Undergrad Enrollment per Completion Rate Category")
# Version 2 - binned data
dataDF_14_15_cleanSize %>%
  ggplot( aes(fill = Size, y = Size, x = Comp.Rate.17_18.BIN)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("School Size per Completion Rate Category") + 
  theme(axis.text.y = element_blank()) +
  xlab("Completion Rate")
# Version 3 - binned data
dataDF_14_15_cleanSize %>%
  ggplot( aes(fill = Comp.Rate.17_18.BIN, y = Comp.Rate.17_18.BIN, x = Size)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("School Size per Completion Rate Category") + 
  theme(axis.text.y = element_blank()) +
...
dataDF_14_15_cleanCost %>%
  ggplot( aes(x = Comp.Rate.17_18.BIN, y = Average.Cost.per.Academic.Year, fill = Comp.Rate.17_18.BIN)) +
  geom_boxplot() +
  ggtitle("Average Cost/Year per Completion Rate Category")

### Visualize: Out of State Tuition & Fees
dataDF_14_15 %>%
  ggplot( aes(x = Comp.Rate.17_18.BIN, y = Out.of.State.Tuition.and.Fees, fill = Comp.Rate.17_18.BIN)) +
  geom_boxplot() +
  ggtitle("Out of State Costs per Completion Rate Category")
...
dataDF_14_15_cleanRegion %>%
  ggplot( aes(fill = Region, y = Region, x = Comp.Rate.17_18.BIN)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("Region per Completion Rate Category") + 
  theme(axis.text.y = element_blank()) +
  xlab("Completion Rate")
# Version 2
dataDF_14_15_cleanRegion %>%
  ggplot( aes(fill = Comp.Rate.17_18.BIN, y = Comp.Rate.17_18.BIN, x = Region)) + 
  geom_bar(position="stack", stat="identity") +
  ggtitle("Region per Completion Rate Category") + 
  theme(axis.text.y = element_blank()) +
  ylab("Completion Rate")
...
More:
- Bayesian probabilities? Ex: Prob of completing college given ____.
- permutation test ??
- something about confidence intervals
